...
namespace CustomInspector.Extensions
{
    public static class DrawProperties
    {
        ...
        public static class ListDrawer
        {
            ...
            public static Rect GetHeaderRect(Rect original)
            {
                original.height = HeaderHeight;
                return original;
            }
            /// <summary>
            /// Accepts dropping objects of given type. if area is clicked, property changes its isExpanded
            /// </summary>
            /// <param name="position"></param>
            /// <param name="targetType"></param>
            /// <param name="property">Toggles it isExpanded on click</param>
            /// <returns></returns>
            public static IEnumerable<Object> AcceptDragNDrop(Rect position, Type targetType, SerializedProperty property)
            {
                // draw invisible button: forces an update on hover
                using (new GUIColorScope(new Color(0, 0, 0, 0)))
                {
                    if (GUI.Button(position, GUIContent.none))
                    {
                        property.isExpanded ^= true; // preserve foldout
                    }
                }
                ...
                {
                    EditorGUI.DrawRect(position, new Color(0.5f, 0.5f, 0.5f, 0.2f));

                    Event evt = Event.current;
                    if (evt.type == EventType.DragUpdated || evt.type == EventType.DragPerform) // if currently dragging
                    {
                        if (DragAndDrop.objectReferences.Any(obj => targetType.IsAssignableFrom(obj.GetType()))
                            || ((typeof(Component).IsAssignableFrom(targetType)) && DragAndDrop.objectReferences.Any(obj => obj is GameObject gob && gob.GetComponent(targetType) != null)))
                        {
                            DragAndDrop.visualMode = DragAndDropVisualMode.Copy; // display the "allow drop" icon

                            if (evt.type == EventType.DragPerform) //if mouse-button up
                            {
                                DragAndDrop.AcceptDrag();
                                foreach (Object draggedObject in DragAndDrop.objectReferences)
                                {
                                    if (draggedObject == null) // deleted components are unity-null
                                        continue;

                                    if (targetType.IsAssignableFrom(draggedObject.GetType()))
                                    {
                                        var casted = Convert.ChangeType(draggedObject, targetType);
                                        yield return (Object)casted;
                                    }
                                    else if (draggedObject is GameObject gob)
                                    {
                                        if (gob.TryGetComponent(targetType, out Component c))
                                        {
                                            var casted = Convert.ChangeType(c, targetType);
                                            yield return (Object)casted;
                                        }
                                    }
                                }
            ...
            public static void DrawList(bool isReadOnly, Rect position, GUIContent label, SerializedProperty property)
            {
                ...
                using (new NewIndentLevel(0))
                {
                    DrawHeader(GetHeaderRect(position), label, property);
